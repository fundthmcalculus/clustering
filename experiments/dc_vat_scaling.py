...
from tribbleclustering.pcvat import (  # noqa: E402
    vat_prim_mst_c, compute_ivat_c, pairwise_distances_c_64,
)
from experiments.blockwise_vat import (  # noqa: E402
    make_blobs, partition, ivat_image_from_order,
...
def run():
    ...
    for ci, n in enumerate(SIZES):
        ...
        for ri, N in enumerate(NS):
            ...
            if n <= QUALITY_MAX_N:
                ...
                naive_ari[ri, ci] = adjusted_rand(labels_from_order(o_naive, img, 20), lbl)
                o_st = stitched_vat(D, X, N, n_repr=24, seed=7)
                img_st = ivat_image_from_order(D, o_st)
                stitched_ari[ri, ci] = adjusted_rand(labels_from_order(o_st, img_st, 20), lbl)
    ...
    return dict(naive_speedup=naive_speedup, naive_ari=naive_ari,
                stitched_ari=stitched_ari, exact_ms=exact_ms,
                bor_cpu_ms=bor_cpu_ms, bor_gpu_ms=bor_gpu_ms)
...
def _plot_speedup_heatmap(sp):
    ...
    ax.set_xlabel("n (samples)"); ax.set_ylabel("N (partition size)")
    ax.set_title("Naive block-decomposition VAT — ideal-parallel speedup\n"
...
def _plot_quality_heatmaps(naive_ari, stitched_ari, qmax):
    ...
    for ax, M, title in ((axes[0], naive_ari, "naive block-decomposition"),
                         (axes[1], stitched_ari, "structure-aware + stitch")):
        sub = M[:, :ncol]
        im = ax.imshow(sub, cmap="RdYlGn", vmin=0, vmax=1, aspect="auto")
        ax.set_xticks(range(ncol)); ax.set_xticklabels(SIZES[:ncol])
        ax.set_yticks(range(len(NS))); ax.set_yticklabels(NS)
        ax.set_xlabel("n (samples)"); ax.set_ylabel("N (partition size)")
        ax.set_title(f"ARI vs truth — {title}")
        _annot(ax, sub, "{:.2f}")
        fig.colorbar(im, ax=ax, label="ARI (1=exact clustering)")
    fig.suptitle("Cluster quality across scale x partition size "
                 "(naive degrades with N; stitch stays ~exact)", fontsize=12)
    fig.tight_layout()
...
def _plot_spectrum(exact, bcpu, bgpu, naive8):
    ...
    ax.set_xlabel("n (samples)"); ax.set_ylabel("MST-build time (ms)")
    ax.set_title("Divide-and-conquer VAT spectrum\n"
